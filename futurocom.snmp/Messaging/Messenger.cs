...
namespace futurocom.snmp.Messaging
{
    ...
    public static class Messenger
    {
        ...
        public static IList<Variable> Set(VersionCode version, IPEndPoint endpoint, OctetString community, IList<Variable> variables, int timeout)
        {
            ...
            if (pdu.ErrorStatus.ToInt32() != 0)
            {
                ResponseMessage mes = response as ResponseMessage;
                if (mes != null)
                    throw ErrorException.Create("Error : " + mes.ErrorStatus.ToString(),
                        endpoint.Address,
                        response);
                else
                {
        ...
        public static void SendTrapV2(int requestId, VersionCode version, EndPoint receiver, OctetString community, ObjectIdentifier enterprise, uint timestamp, IList<Variable> variables)
        {
            ...
            var message = new TrapV2Message(
                requestId,
                receiver is IPEndPoint ? ((IPEndPoint)receiver).Address : null,
                version, 
                community, 
                enterprise, 
                timestamp, 
        ...
        /// <param name="report">The report.</param>
        [CLSCompliant(false)]
        public static void SendInform(int requestId, 
            VersionCode version, 
            IPEndPoint receiver, 
            OctetString community, 
