...
namespace futurocom.snmp.Messaging
{
    ...
    public static class MessageFactory
    {
        ...
        /// <returns></returns>
        public static IList<ISnmpMessage> ParseMessages(EndPoint sourceEndPoint, IEnumerable<char> bytes, UserRegistry registry)
        {
            if (bytes == null)
            {
            ...
            return ParseMessages(sourceEndPoint, ByteTool.Convert(bytes), registry);
        }
        ...
        /// <param name="registry">The registry.</param>
        /// <returns></returns>
        public static IList<ISnmpMessage> ParseMessages(EndPoint sourceEndPoint, byte[] buffer, UserRegistry registry)
        {
            if (buffer == null)
            {
            ...
            return ParseMessages(sourceEndPoint, buffer, 0, buffer.Length, registry);
        }
        ...
        /// <param name="registry">The registry.</param>
        /// <returns></returns>
        public static IList<ISnmpMessage> ParseMessages(EndPoint sourceEndPoint, byte[] buffer, int index, int length, UserRegistry registry)
        {
            if (buffer == null)
            {
                throw new ArgumentNullException("buffer");
            ...
            {                
                ...
                {
                    var message = ParseMessage(sourceEndPoint, first, stream, registry);
                    if (message == null)
                    {
                        continue;
                    }
        ...
        private static ISnmpMessage ParseMessage(EndPoint sourceEndPoint, int first, Stream stream, UserRegistry registry)
        {
            var array = DataFactory.CreateSnmpData(sourceEndPoint, first, stream);
            ...
            else
            {
                ...
                parameters = new SecurityParameters(sourceEndPoint, (OctetString)body[2]);
                ...
                else if (code == SnmpType.OctetString)
                {
                    ...
                    try
                    {
                        scope = new Scope((Sequence)privacy.Decrypt(sourceEndPoint, body[3], parameters));
                    }
            ...
            switch (scopeCode)
            {
                case SnmpType.TrapV1Pdu:
                    return new TrapV1Message(sourceEndPoint is IPEndPoint ? ((IPEndPoint)sourceEndPoint).Address : null,body);
                case SnmpType.TrapV2Pdu:
                    return new TrapV2Message(
                        version, 
                        sourceEndPoint is IPEndPoint?((IPEndPoint)sourceEndPoint).Address:null,
                        header, 
                        parameters, 
                        scope, 
                        privacy);
                ...
                case SnmpType.InformRequestPdu:
                    return new InformRequestMessage(
                        version,
                        sourceEndPoint is IPEndPoint ? (IPEndPoint)sourceEndPoint : null,
                        header, 
                        parameters, 
                        scope, 
                        privacy);
