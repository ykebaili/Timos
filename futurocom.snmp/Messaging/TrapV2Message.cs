...
namespace futurocom.snmp.Messaging
{
    ...
    public class TrapV2Message : ISnmpMessage
    {
        ...
        [CLSCompliant(false)]
        public TrapV2Message(int requestId, IPAddress senderIP, VersionCode version, OctetString community, ObjectIdentifier enterprise, uint time, IList<Variable> variables)
        {
            if (variables == null)
            {
        ...
        public IPrivacyProvider Privacy { get; protected set; }

        /// <summary>
        /// Gets the parameters.
        /// </summary>
        /// <value>The parameters.</value>
        public SecurityParameters Parameters { get; protected set; }
        ...
        /// </returns>
        [CLSCompliant(false)]
        public virtual string ToString(IObjectRegistry objects)
        {
            return string.Format(
                CultureInfo.InvariantCulture,
                "SNMPv2 trap: time stamp: {0}; sender {4}; community: {1}; enterprise: {2}; varbind count: {3}",
                TimeStamp.ToString(CultureInfo.InvariantCulture),
                this.Community(),
                Enterprise.ToString(objects),
                this.Variables().Count.ToString(CultureInfo.InvariantCulture),
                SenderIP != null?SenderIP.ToString():"?");
        }

        internal string GetTraceInfo()
        {
            return ToString();
        }
    }
