...
namespace futurocom.snmp
{
    ...
    public class TrapV1Pdu : ISnmpPdu
    {
        ...
        public TrapV1Pdu(EndPoint sourceEndPoint, Stream stream)
        {
            if (stream == null) 
            {
            ...
            Enterprise = (ObjectIdentifier)DataFactory.CreateSnmpData(sourceEndPoint, stream);
            AgentAddress = (IP)DataFactory.CreateSnmpData(sourceEndPoint, stream);
            _generic = (Integer32)DataFactory.CreateSnmpData(sourceEndPoint, stream);
            _specific = (Integer32)DataFactory.CreateSnmpData(sourceEndPoint, stream);
            TimeStamp = (TimeTicks)DataFactory.CreateSnmpData(sourceEndPoint, stream);
