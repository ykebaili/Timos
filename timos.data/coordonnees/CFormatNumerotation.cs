...
namespace timos.data
{
	...
	[Table(CFormatNumerotation.c_nomTable, CFormatNumerotation.c_champId, true )]
	[FullTableSync]
	[ObjetServeurURI("CFormatNumerotationServeur")]
    ...
    public class CFormatNumerotation : CObjetDonneeAIdNumeriqueAuto, 
		...
		IComparer
	{
		...
		/// /////////////////////////////////////////////
		public CFormatNumerotation( CContexteDonnee contexte)
			:base(contexte)
		{
		}
		...
		{
			get	{ return I.T("Numbering format @1|437",Libelle); }
		}
		...
		/// </summary>
		[TableFieldProperty(c_champSequence, 255)]
		...
		/// </summary>
		[TableFieldProperty(c_champLongueurReference)]
		...
		/// </summary>
		[TableFieldProperty(c_champRomain)]
		...
		/// <summary>
		/// Retourne le nombre de Systemes de Coordonnées utilisant le Format de Numérotation
		/// </summary>
		public int NbSystemeCoordonneesUtilisantCeFormatDeNumerotation
		{
			get
			{
				List<CSystemeCoordonnees> lstSC = new List<CSystemeCoordonnees>();
				//On récupère les CRSCFN des systemes de coordonnées pointant vers ce Format de Numérotation
				CListeObjetsDonnees lstcrscfn = GetDependancesListe(CRelationSystemeCoordonnees_FormatNumerotation.c_nomTable, CFormatNumerotation.c_champId);
				foreach (CRelationSystemeCoordonnees_FormatNumerotation crscfn in lstcrscfn)
					if (lstSC.IndexOf(crscfn.SystemeDeCoordonnees) == -1)
						lstSC.Add(crscfn.SystemeDeCoordonnees);
		...
		/// <summary>
		/// Permet de savoir si ce format de numérotation n'est pas utilisé par un
		/// <see cref="CSystemeCoordonnees">Système de Coordonnees</see> et peut donc être modifié
		/// </summary>
		public bool CanEdit
		{
			get
			{
				if (NbSystemeCoordonneesUtilisantCeFormatDeNumerotation == 0)
					return true;
				else
					return false;
			}
