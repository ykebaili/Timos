...
{
    /// <summary>
    /// Classe récupérée de http://www.codeproject.com/KB/printing/datagridviewprinter.aspx
    /// </summary>
    class DataGridViewPrinter
    {
            ...
            // Just calculate once
            {
                ...
                for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                {
                    ...
                    for (int j = 0; j < TheDataGridView.Rows.Count; j++)
                    {
                        ...
                        object obj = TheDataGridView.Rows[j].Cells[i].Value;
        ...
        public bool DrawDataGridView(Graphics g)
        {
            ...
            {
                CFormAlerte.Afficher(I.T("Operation failed : |30206") + ex.Message.ToString(), EFormAlerteType.Erreur);
