...
namespace timos.supervision
{
	...
	public class TreeListView : Control, ISupportInitialize
	{
		...
		public timos.supervision.HitInfo CalcColumnHit(Point mousepoint)
		{
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			timos.supervision.HitInfo info = Columns.CalcHitInfo(new Point(e.X, e.Y), HScrollValue());
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				p.Style &= ~(int)timos.supervision.WinUtil.WS_BORDER;
				p.ExStyle &= ~(int)timos.supervision.WinUtil.WS_EX_CLIENTEDGE;
        ...
        protected virtual void PaintRowHeader(Graphics g, Rectangle rc, Node node, bool isHot)
        {
            if (m_rowPainter != null)
                m_rowPainter.DrawHeader(g, rc, isHot);
        }
