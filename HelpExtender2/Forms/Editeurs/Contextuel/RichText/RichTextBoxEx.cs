...
namespace HelpExtender
{
	...
	{
		#region :[ Propriétés ]:
		private static string c_baliseLnkStart = "#LNKSTART";
		private static string c_baliseLnkEnd = "#LNKEND";
		#endregion
		...
		public void InsertLink(string text, string hyperlink, int position)
		{
			...
			string strTextLink = @"{\rtf1\ansi " + GetLinkRtf(text, hyperlink) + "}";
			this.SelectionStart = position;
			this.SelectedRtf = strTextLink;
			int nLength = text.Length + hyperlink.Length + c_baliseLnkEnd.Length + c_baliseLnkStart.Length + 2;
			this.Select(position, nLength);
			this.SetSelectionLink(true);
			this.Select(position + nLength, 0);
			//this.Select(position + "LNKSTART".Length+text.Length + hyperlink.Length + 1, 0);
		}
		...
		private string GetLinkRtf(string strText, string strLink)
		{
			return @"\v " + c_baliseLnkStart + @" \v0" + strText + @"\v " + c_baliseLnkEnd + strLink + @"\v0";
		}

		public void RecreeLinks()
		{
			string strRTF = this.Rtf;
			//string strParts = strRTF.Split("\v0 "+c_baliseLnkStart);
