...
begin
    ...
    -- Contrôle de la date. 10/03/2010, des equipements ERECA donnent une date erronee
    -- telle que (ED57 01 13 10:35:34 a la place de 2010 01 13 10:35:34), ce qui
    -- plante le client SPV lorsqu'il reçoit le message d'alarme. Lorsque la date
    -- n'est pas conforme, on la remplace par la date systeme.
    begin
	select to_date(:new.ALARM_DATE, 'YYYY MM DD HH24:MI:SS') into datebidon from dual;
    exception
	when others then
            insert into error values (seq_error.nextval, 0, :new.alarm_id || ' date non conforme ' || :new.alarm_date);
	    select to_char (sysdate, 'YYYY MM DD HH24:MI:SS') into :new.ALARM_DATE from dual;
    end;
...
Fin JPB */
 ...
 --   insert into test values (seq_test.nextval, 'tib_alarm1 :new.ALARM_Grave = '|| :new.ALARM_Grave);
    ...
    if (NumeroIana is not null and NumeroIana = c_iana_nec and :new.ALARM_CL = 'TRAPS') then --NEC, pour les traps on prend la gravité dans le trap
...
--	:new.ALARM_LOCAL   := 0;
    ...
    if (AlarmIdDeb is null) then    -- Début d'alarme
        insert into ALARMDATA (ALARMDATA_ID, ALARMDATA_DATEDEBUT, ALARMDATA_SECDEBUT, SITE_ID, EQUIP_ID, LIAI_ID, ALARMDATA_IANA,
                    ...
                    values (AlarmeId, AlarmDate, AlarmSec, SiteId, EquipId, LiaiId, NumeroIana,
